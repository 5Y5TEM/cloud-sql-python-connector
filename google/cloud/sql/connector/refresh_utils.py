...
async def _is_valid(task: asyncio.Task) -> bool:
    try:
        ...
        if datetime.now(timezone.utc) < metadata.expiration:
            return True
    except Exception:
